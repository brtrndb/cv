% !TEX encoding = UTF-8 Unicode
% !TEX TS-program = LuaLaTeX

%% ModernCV.
\documentclass[10pt, a4paper]{moderncv}
\usepackage{luatextra}
\usepackage[french]{babel}
\usepackage[scale=0.85]{geometry}
\usepackage{fontawesome}
\usepackage{wrapfig}
\usepackage{ifthen}

%% ModernCV document style.
\moderncvstyle{classic}                         %% Styles: classic, casual, oldstyle, banking, fancy.
\moderncvcolor{blue}                            %% Colors: blue, orange, green, red, purple, grey, black.
\moderncvicons{awesome}                         %% Icons: marvosym, awesome, letters (banking only).
\renewcommand{\familydefault}{\sfdefault}
\nopagenumbers{}

%% Main colors of moderncv template.
\definecolor{modernBlue}{rgb}{0.22,0.45,0.70}   % Light blue.
\definecolor{modernGreen}{rgb}{0.35,0.70,0.30}  % Green.
\definecolor{modernGrey}{rgb}{0.55,0.55,0.55}   % Dark grey.
\definecolor{modernGrey2}{rgb}{0.45,0.45,0.45}  % Dark grey 2.
\definecolor{modernGrey3}{rgb}{0.35,0.35,0.35}  % Dark grey 3.
\definecolor{modernOrange}{rgb}{0.95,0.55,0.15} % Orange.
\definecolor{modernPurple}{rgb}{0.50,0.33,0.80} % Purple.
\definecolor{modernRed}{rgb}{0.95,0.20,0.20}    % Red.

%% Personnal informations.
\name{\textbf{Bertrand}}{\textbf{Boyer}}
\title{Ingénieur Innovation\newline{}Développeur C/C++/Java}
\address{}{}{}
\phone[mobile]{+33 (0) 6 10 04 27 16}
%% \phone[fixed]{}
%% \phone[fax]{}
\email{boyer.bertrand@gmail.com}
%% \homepage{}
\social[linkedin]{BertrandBoyer}
\social[github]{BertrandBoyer}
%% \social[twitter]{}
%% \photo[64pt][0.5pt]{}
\extrainfo{Diplômé Epitech Promotion 2015}
%% \quote{}

\begin{document}

%% Header.
\makecvtitle

%% Education section.
\newcommand{\displayEducation}{yes}
\ifthenelse{\equal{\displayEducation}{yes}}{\input{./CV-Education.tex}}

%% Experiences section.
\newcommand{\displayExperiences}{yes}
\newcommand{\displayVisianMissions}{yes}
\newcommand{\displaySIIMissions}{yes}
\newcommand{\displayThalesMissions}{yes}
\newcommand{\displayNovacomMissions}{yes}
\ifthenelse{\equal{\displayExperiences}{yes}}{\input{CV-Experiences.tex}}

%% Skills section.
\newcommand{\displaySkills}{yes}
\ifthenelse{\equal{\displaySkills}{yes}}{\input{CV-Skills.tex}}

%% Section Projets.
\newcommand{\displayProject}{no}
\ifthenelse{\equal{\displayProject}{yes}}{\input{CV-Projects.tex}}{}

%% Languages section.
\newcommand{\displayLanguages}{yes}
\ifthenelse{\equal{\displayLanguages}{yes}}{\input{CV-Languages.tex}}

%% Hobbies section.
\newcommand{\displayHobbies}{yes}
\ifthenelse{\equal{\displayHobbies}{yes}}{\input{CV-Hobbies.tex}}

\end{document}
