...
\setlength{\cvSectionSpace}{-0.50 em}
\newlength{\defaultEntrySpace} %% Default vertical space between each cv entry.
\setlength{\defaultEntrySpace}{+0.25 em}
\newlength{\skillEntrySpace} %% Vertical space between each skill entry.
\setlength{\skillEntrySpace}{\defaultEntrySpace}
\newlength{\experienceEntrySpace} %% Vertical space between each experience entry.
\setlength{\experienceEntrySpace}{2\defaultEntrySpace}
\newlength{\educationEntrySpace} %% Vertical space between each education entry.
\setlength{\educationEntrySpace}{\defaultEntrySpace}
\newlength{\extraEntrySpace} %% Vertical space between each extra entry.
\setlength{\extraEntrySpace}{\defaultEntrySpace}
...
% #1: icon, #2: group name, #3: content.
  \cvitem[\skillEntrySpace]{\skillGroup{#1}{#2}}{#3}%
}
...
% #1: company name, #2: location, #3: remote icon, #4: start date, #5: end date, #6: job title, #7: tags, #8: content.
  ...
  {\hfill\raisebox{\baseletterheight}{\color{color1}\rule{0.33\hintscolumnwidth}{0.25 ex}}\newline\jobInfo{#4}{#5}{#2}[#3]}
...
% #1: url, #2: logo, #3: job description, #4: job items, #5: skills.
  \IfBlankTF{#2}{%
    \parbox{0.80\maincolumnwidth}{%
      \IfBlankF{#3}{\jobDescription{#3}\IfBlankTF{#4}{\IfBlankF{#5}{\newline}}{\newline}}
      \IfBlankF{#4}{\jobItems{#4}\IfBlankF{#5}{\newline}}
...
\newcommand{\jobItems}[1]{
  ...
  \foreach \x in {#1}{%
    ...
    \textcolor{color1}{\hspace{+0.20 em}\faAngleRight}\x\ifthenelse{\NOT\equal{\value{CurrentListMemberCount}}{\value{TotalNumberOfListMembers}}}{\newline}{}%
  }%
}
...
% #1: school name, #2: start date, #3: end date, #4: icon, #5: description, #6: location, #7: content.
  \cventry[\educationEntrySpace]
  {#4\hfill\IfBlankTF{#2}{#3}{\schoolDate{#2}{#3}}}
  {\schoolName{#1}}
  {#5}
  {\schoolLocation{#6}}
  {}
...
% #1: icon, #2: description.
  \hspace{-0.40\hintscolumnwidth}\cvitem[\extraEntrySpace]{#1}{\hspace{-1.00 em}#2}%
}
